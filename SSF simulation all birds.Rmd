...
for (i in 1:length(forest.raster.cropped)){
  ...
  #PROBLEM WITH i 10 FREQUENCY 881: The id of the patch according to Adam is 26, and the focal patch of the labeled full raster mapo is 96. the patch is very big. Ask Adam if that is correct. Maybe the raster map does not have the splits between patches so it is considering a big patch as only one
  #PROBLEM WITH i 11 FREQUENCY 942: The id of the patch according to Adam is 53, and the focal patch of the labeled full raster mapo is 96. the patch is very big. Ask Adam if that is correct. Maybe the raster map does not have the splits between patches so it is considering a big patch as only one
  #PROBLEM WITH i 13 FREQUENCY 392: The id of the patch according to Adam is 32, and the focal patch of the labeled full raster mapo is 96. the patch is very big. Ask Adam if that is correct. Maybe the raster map does not have the splits between patches so it is considering a big patch as only one
  #PROBLEM WITH i  F17REQUENCY 631: The id of the patch according to Adam is 32, and the focal patch of the labeled full raster mapo is 96. the patch is very big. Ask Adam if that is correct. Maybe the raster map does not have the splits between patches so it is considering a big patch as only one
  
###***SEE WHY IT IS NOTOT ESTIMATING WELL THE PROPORTION OF FOREST IN EACH LANDSCAPE
  #WRITE DOWN THE INSTRUCTION FOR THE CODE I JUST WROTE
...
#for (i in 1:nrow(parameters.simulations)){
  ...
  results.simulations.list <- foreach (i= c(1,2),#1:6,8,9,11:20,22:23), #c(1:6,8,9,11:20,22:23), #11 #1:nrow(parameters.simulations),
