...
#***CONTINUE CHECKING WHY I CANNOT RUN SOME BIRDS
View(condensed.dataframe.subset.remaining[c(1),] %>% 
  mutate(fit = map(ssf, ~ fit_issf(.,case_ ~ forest27_end + log_sl_ + cos(ta_) + strata(step_id_)))))
...
```{r}
#THIS IS SOME TEST CODE TO CHECK IF I DID THE RIGHT THING
test.map<-crop(land_use_raster, amt::bbox(condensed.dataframe.subset$trk[[i]], spatial = TRUE, buff = 200))

points.inside<-matrix(data=c(276500,971000),nrow=1)
  
points.outside<-matrix(data=c(275500,972200),nrow=1) 

forest.raster.cropped[[i]]$forest.raster.cropped.labeled[cellFromXY (forest.raster.cropped[[i]]$forest.raster.cropped.labeled, points.inside)]

forest.raster.cropped[[i]]$forest.raster.cropped.labeled[cellFromXY (forest.raster.cropped[[i]]$forest.raster.cropped.labeled, points.outside)]
...
results.emigration %>% 
  ...
  ungroup() %>%  
  ggplot(aes(x=factor(fate,level=c("prop.same.patch","prop.matrix","prop.new.patch")),y=mean.prop,fill=fate)) +
  geom_col() +
  geom_errorbar(aes(ymin=mean.prop-std.dev, ymax=mean.prop+std.dev), width=.2, position=position_dodge(.9)) +
  xlab("Fate") +
