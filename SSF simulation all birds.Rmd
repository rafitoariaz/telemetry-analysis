...
#T. test for differences in turning angle by vegetal cover
t.test(tracking.database.unlisted %>% select(forest27_start,ta_) %>% filter(forest27_start=="matrix") %>% select(ta_) %>% filter(ta_!=0) %>% .$ta_,tracking.database.unlisted %>% select(forest27_start,ta_) %>% filter(forest27_start=="forest") %>% select(ta_) %>% filter(ta_!=0) %>% .$ta_)
...
parameters.simulations.remaining <- data.frame(id.simulation=Vi$id) #*** IN THIS LINE I WOULD PUT THE NUMBER OF SIMULATIONS I WOULD LIKE TO DO, BUT NO I AM PUTTING THE ID OF THE BIRD SINCE EACH ONE HAS A SPECIFIC PARAMTER
...
#pgamma modifying the shape paramater by a certain value

jpeg("Graphs/Gamma distribution modification shape parameter.jpg",quality=100,width=1000,height=700)
par(mfrow=c(2,2))

#Generate a vector that will increase the shape parameter by a certain value
increase.shape<-1:4

for (k in increase.shape){

for (i in 1:nrow(parameters.simulations)){
plot(c(min(tracking.database.unlisted$sl_):max(tracking.database.unlisted$sl_)), pgamma(min(tracking.database.unlisted$sl_):max(tracking.database.unlisted$sl_), shape = parameters.simulations[i,"shape.step.length"]*k, scale = parameters.simulations[i,"scale.step.length"]), type = "l",col=i,ylim=c(0,1),xlab="Dispersal distance (m)",ylab="Probability",main=paste("Original shape parameter *",k))
...
#Boxplot of values of the shape parameter
parameters.simulations %>% 
  ggplot(aes(y=shape.step.length)) +
  geom_boxplot()

#Summary of values of the shape parameter
summary(parameters.simulations$shape.step.length)


##Generate a data frame that will increase the shape parameter by a certain value
increase.shape<-data.frame(increase=1:4)

#Obtain the mean of the shape parameter and increase it by a certain value
increase.shape$shape.parameter <- mean(parameters.simulations$shape.step.length)*increase.shape$increase

#Use the mean of the scale parameter
increase.shape$scale.parameter <- mean(parameters.simulations$scale.step.length)

jpeg("Graphs/Gamma distribution modification mean shape parameter.jpg",quality=100,width=1000,height=700)
for (i in 1:nrow(increase.shape)){
plot(c(min(tracking.database.unlisted$sl_):max(tracking.database.unlisted$sl_)), pgamma(min(tracking.database.unlisted$sl_):max(tracking.database.unlisted$sl_), shape = increase.shape[i,"shape.parameter"]*k, scale = increase.shape[i,"scale.parameter"]), type = "l",col=i,ylim=c(0,1),xlab="Dispersal distance (m)",ylab="Probability",main="Modification of mean shape parameter")
  
  ifelse(i==nrow(increase.shape),par(new=FALSE),par(new=TRUE))
}

legend("bottomright",legend=increase.shape$increase,col=1:length(increase.shape),lty=1)
...
results.emigration %>% 
  select(-c(focal.patch,matrix,new.patch,same.patch)) %>% 
...
borrar %>% 
  ggplot(aes(y=log.dispersal.distance)) +
  geom_boxplot() +
  facet_wrap(~fate)

#Summary of seed dispersal distance by fate
summary(borrar$dispersal.distance)
...
#Model without random structure
#library("nlme")
summary(mod.1<-gls(dispersal.distance ~scale(param.shape.step.length) + scale(param.habitat.utilization) + scale(time),method = "REML", data = results.simulations.dispersal.distance.df %>% 
  filter(same.patch==1 | new.patch==1)))


#GLMM nesting frequency
summary(mod.2<-lme(dispersal.distance ~scale(param.shape.step.length) + scale(param.habitat.utilization) + scale(time),random = ~1 | id.simulation,data=results.simulations.dispersal.distance.df %>% 
  filter(same.patch==1 | new.patch==1),method="REML"))

#Continue with LMM
#Without habitat utilization
summary(mod.3 <- update(mod.2,.~. -scale(param.habitat.utilization)))
