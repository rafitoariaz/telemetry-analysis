...
for (i in 1:nrow(condensed.dataframe.subset)) {
  ...
  condensed.dataframe.subset$shape.step.length[[i]] <- sl_shape(condensed.dataframe.subset$fit[[i]])  #It uses a gamma distribution for step length. I think this is the mean. ***Understand why are they using a gamma distribution
  condensed.dataframe.subset$scale.step.length[[i]] <- sl_scale(condensed.dataframe.subset$fit[[i]]) #It uses a gamma distribution for step length. I think this is the scale, which is part of what will determine the distribution ***Understand why are they using a gamma distribution

  }
  
#Make a list with the forest raster cropped to the extent of the animal that moved
structure <- list(id.freq=numeric(),forest.raster.cropped=list(),movement.kernel=list())
...
for (i in 1:length(forest.raster.cropped)){
  ...
  #Crop the map to the area in which the animal moved
  ...
  forest.raster.cropped[[i]]$movement.kernel <- movement_kernel(condensed.dataframe.subset$scale.step.length[[i]], condensed.dataframe.subset$shape.step.length[[i]], forest.raster.cropped[[i]]$forest.raster.cropped)
  
  #Store habitat kernel
  forest.raster.cropped[[i]]$habitat.kernel <- habitat_kernel(list(forest27 = coef(condensed.dataframe.subset$fit[[i]])["forest27_endforest"]), forest.raster.cropped[[i]]$forest.raster.cropped)
...
for (i in 1:length(forest.raster.cropped)){
  ...
  forest.raster.cropped[[i]]$focal.patch <- subset(frequencies.patches,frequencies.patches$FREQ==forest.raster.cropped[[i]]$id.freq)[,c("X","Y")]
  
  #Obtain the cell number from the xy cordinate
  forest.raster.cropped[[i]]$focal.patch <- forest.raster.cropped[[i]]$forest.raster.cropped.labeled [cellFromXY (forest.raster.cropped[[i]]$forest.raster.cropped.labeled, forest.raster.cropped[[i]]$focal.patch)]
