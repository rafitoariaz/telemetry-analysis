...
parameters.simulations.model <- data.frame(shape.step.length=quantile(parameters.simulations$shape.step.length,probs=c(0.05,0.5,0.95)),
             ...
             habitat.utilization=quantile(parameters.simulations %>% filter(habitat.utilization>-5) %>% select(habitat.utilization) %>% .$habitat.utilization,probs=c(0.05,0.5,0.95))) %>% 
...
#1,2,4,8,13,15,17,18
#for (i in 1:nrow(parameters.simulations)){
results.simulations.list <- foreach (i= c(1),#c(1,2,4,9,11,13,14),
...
#The structure of the resulting list is the following:
#results.simulations.list[[a]]=slot for the simulations
