...
for (i in 1:2){
#results.simulations <- foreach (i=c(4,8), .packages=c("amt","raster")) %dopar% {
...
results.simulations.2 <- foreach (Patch=focal.patch.database[c(1,2,4)],
                               .packages=c("amt","raster")) %dopar% {
simulate.movement(focal.patch=Patch$focal.patch.label,
                  tot.num.minutes=10,
                  fix.rate=5,
                  parameters.simulations=parameters.simulations,
                  num.sim.points.per.patch=num.sim.points.per.patch, #I tried to only put a number and then save the number I put with the name of the variable but I could not do it, so that is why I specifya outside the function the value and store in in the variable
...
#for (i in 1:nrow(parameters.simulations)){
results.simulations.2 <- foreach (Patch=focal.patch.database[c(1,2)],
                               .packages=c("amt","raster")) %dopar% {
a = as.character(Patch$id.freq)
i <- which(names(forest.raster.cropped) ==a)

#cat(c(a, i), file=stdout())
simulate.movement(focal.patch=Patch$focal.patch.label,
                  tot.num.minutes=10,
                  fix.rate=5,
                  parameters.simulations=parameters.simulations,
...
#for (i in 1:nrow(parameters.simulations)){
results.simulations <- foreach (i=1:2,
                                .packages=c("amt","raster")) %dopar% {
...
#for (i in 1:nrow(parameters.simulations)){
foreach (i=1,.packages=c("amt","raster"),.combine="output.results.simulations.function",.init=i,.multicombine=TRUE) %dopar% {
