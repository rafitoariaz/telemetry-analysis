...
#PENDING ACTIVITIES
  #When I am simulating the movement behavior of animals, sometimes the next point lands in a coordinate where there is no information in the raster map. I did a code that it repeats the simulation of calculating a new point every time it lands outside the raster map and it is working when I am running the code individually. However, when I run the code for doing the calculations in parallel or in different cores, it is still not working and I do not know why
  #When I use the movement_kernel function in the amt package, I cannot fix a specific value for the von Misses distribution parameter, so I do not know very well what value is taking when I use the function. 
  #When I estimate the ssf, it generates random steps to compare them with the actual steps. The problem is that every time I run the code from scratch it makes new random steps, so the parameters in the SSF are different each time
  #I need to set.seed in the simulation code of the movement behavior. 

#Problems
*When I do the simulations with the movement_kernel function in amt package, I cannot fix the von Misses distribution for the turning angle, so I do not know
*Line 216. I had to update the dplyr package so that I could run a code but now the filter_min_n_burst function is not working. I will ignore it until I fix it
*I am using Adam's raster map for knowing which areas are forest an non-forest, but I am using Felipe's shp file for identifying patch id. They might be different, so one solution would be to process Felipe's shp file so that it gives me information of the forested and non-forested areas
...
rm(clean.freq,clean.tel.data,prueba,m,d)
```
...
crs.adam<-crs(land_use_raster)

rm(m)
```

```{r}
#Prepare Felipe's raster map
#Read shape file
s<-sf::st_read(dsn="Data bases/raster_map_costa_rica_felipe/Forest.shp")

# Rasterize field 'PatchName'

#Note: I defined the background value as '0' (not NA), this will be the matrix. I'm setting the function to 'min' so that it takes the minimum PatchName if there are multiple patches in a cell. I hope this will give it the matrix value, but I'm not sure it will. Otherwise can use e.g. 'last'.
Raster <- raster(s, res = 10)
Raster <- fasterize(s, Raster, field = "PatchName", fun = "min", background = 0, by = NULL)
names(Raster) <- "forest27"

land_use_raster<-Raster
land_use_raster[land_use_raster>0]<-1
#names(land_use_raster) <- "forest27"

crs(land_use_raster)<-crs.adam
...
#MAKE MAP WITH RASTER MAP AND GOOGLE MAPS
...
#Subset certain individuals since I cannot run the code with certain frequencies.
###***I am subsetting these frequencies because individuals moved in forested and non-forested areas, so I will use this database to analyse habitat selection since I need points in both kind of forests. ****CHECK LATER HOW TO RUN THE CODE WITH ALL FREQUENCIES
...
###***The reminder of the individuals only moved or barely moved in one  kind of habitat, so I cannot run the habitat selection function if there are no points in both habitats. However, I can use this data base to estimate turning angle and step length. 
...
#Good job Luis! :)
#Tasks to do
###Describe what I just did
###Run analysis of SSF to estimate parameter for each of the simulations
###Obtain a data frame that will have all the information of each of the simulations
###Obtain statistics :)
...
condensed.dataframe.subset.list<-split(condensed.dataframe.subset$id,condensed.dataframe.subset$id)

#Making a data frame of the number of random steps I want to be generated
num.random.steps<-c(seq(from=5,
                        to=30,
                        by=5))
...
#For each frequency
for (i in 1:length(condensed.dataframe.subset.list)){
  
  #Generate a list inside the selected frequency
  condensed.dataframe.subset.list[[i]]<-list()

  #For each number of random step
  for(k in 1:length(num.random.steps)){
  
    #Generate a list
    condensed.dataframe.subset.list[[i]][[k]]<-list()
...
condensed.dataframe.subset.list[[i]][[k]][[l]] <- condensed.dataframe.subset %>% 
  #Filter the id I will analyze
  filter(id==names(condensed.dataframe.subset.list)[i]) %>% 
...
x %>% 
    ...
    print(paste("Frequency",i,"of",length(condensed.dataframe.subset.list)))
    print(paste(k,"of",length(num.random.steps),"random steps",length(condensed.dataframe.subset.list)))
...
x %>% 
    ...
    #filter_min_n_burst(min_n = 3) %>% #Keep only tracks that have three consecutive locations, which is the minimum requeried for doing the analysis
    steps_by_burst() %>% #Converts the points to tracks
    random_steps(n=9) %>% #Makes 9 random steps
    extract_covariates(land_use_raster, where = "both") %>% #Extracts covariates of vegetation on initial and ending location. In "amt tutorial.pdf" it explains the following: covariate values at the start and the end of a step can also be included in the model as interaction with each other, to test hypotheses of the type: Are animals more likely to stay in a given habitat, if they are already in the habitat?
    mutate(forest27_start = factor(forest27_start, levels = c(0, 1), labels = c("matrix", "forest"))) %>% #factor (forest27_start you have to check the name of the column of the data base that is generated one step before (in this case is forest27_start). Just run the 2 precedent pipe lines and a tibble with the names of the column will appeaar)
    mutate(forest27_end = factor(forest27_end, levels = c(0, 1), labels = c("matrix", "forest"))) %>% 
    mutate(log_sl_ = log(sl_)) #Obtain logarithm of step lenght
...
plot(sort(unique(tracking.database.unlisted %>% mutate(ta_=deg2rad(ta_)) %>% select(ta_) %>% .$ta_)),dvmises(sort(unique(tracking.database.unlisted %>% mutate(ta_=deg2rad(ta_)) %>% select(ta_) %>% .$ta_)),kappa=as.numeric(von.mises.parameters[i,"kappa"])),type="l",col=i)
...
for (i in 1:length(condensed.dataframe.subset.list)){
  ...
  for(k in 1:length(num.random.steps)){
    ...
    for(l in 1:num.simulations){
      ...
      condensed.dataframe.subset.list[[i]][[k]][[l]]<-condensed.dataframe.subset.list[[i]][[k]][[l]] %>% 
  mutate(fit = map(ssf, ~ fit_issf(.,case_ ~ forest27_end + log_sl_ + cos(ta_)  + log_sl_:forest27_end + cos(ta_):forest27_end + strata(step_id_))))
      
      
      df$freq.id<-names(condensed.dataframe.subset.list)[i]
      
      df$num.random.steps<-num.random.steps[k]
      
      df$id.simulation<-l
      
      df$shape.step.length <- sl_shape(condensed.dataframe.subset.list[[i]][[k]][[l]]$fit[[1]])  #It uses a gamma distribution for step length. I think this is the mean. ***Understand why are they using a gamma distribution. HERE ALSO I WILL PUT THE DIFFERENT VALUES FOR THE GAMMA DISTRIBUTION I WOULD LIKE TO SIMULATE
  
  df$scale.step.length <- sl_scale(condensed.dataframe.subset.list[[i]][[k]][[l]]$fit[[1]]) #It uses a gamma distribution for step length. I think this is the scale, which is part of what will determine the distribution ***Understand why are they using a gamma distribution. HERE ALSO I WILL PUT THE DIFFERENT VALUES FOR THE GAMMA DISTRIBUTION I WOULD LIKE TO SIMULATE

  
  df$habitat.utilization <- coef(condensed.dataframe.subset.list[[i]][[k]][[l]]$fit[[1]])["forest27_endforest"] #***HERE ALSO I WILL PUT THE DIFFERENT VALUES FOR THE HABITAT SELECTION COEFFICIENT I WOULD LIKE TO SIMULATE
...
for (i in unique(habitat.selection.parameter.df$parameter)){

print(habitat.selection.parameter.df %>% 
    filter(parameter==i & freq.id!=973) %>% 
  ggplot(aes(x=freq.id,y=value)) +
  geom_boxplot() +
  facet_wrap(~num.random.steps) + 
    ylab(i) +
...
d<-habitat.selection.parameter.df %>% 
  filter(freq.id!=973)

tapply(d$value,d$parameter,summary)

print(habitat.selection.parameter.df %>% 
          filter(parameter=="habitat.utilization" & freq.id!=973) %>% 
          ggplot(aes(x=freq.id,y=value)) +
          geom_boxplot() +
          facet_wrap(~num.random.steps) + 
          ylab(i) +
...
d<-original.database %>% 
  mutate(month=substr(t,7,7)) %>% 
  filter(id==973 & month==8)

plot(crop(land_use_raster,extent(min(d$x),max(d$x),min(d$y),max(d$y))))
points(d$x,d$y)

habitat.selection.parameter.df %>% 
  ggplot(aes(x=num.random.steps,y=value,group=num.random.steps)) +
  geom_boxplot() +
...
for (i in 1:nrow(condensed.dataframe.subset.remaining2)){

print(table(condensed.dataframe.subset.remaining2$ssf[[i]]$forest27_start,condensed.dataframe.subset.remaining2$ssf[[i]]$forest27_end))
  
  plot(crop(land_use_raster,extent(min(condensed.dataframe.subset.remaining2$ssf[[i]]$x1_,condensed.dataframe.subset.remaining2$ssf[[i]]$x2_)-200,max(condensed.dataframe.subset.remaining2$ssf[[i]]$x1_,condensed.dataframe.subset.remaining2$ssf[[i]]$x2_)+200,min(condensed.dataframe.subset.remaining2$ssf[[i]]$y1_,condensed.dataframe.subset.remaining2$ssf[[i]]$y2_)-200,max(condensed.dataframe.subset.remaining2$ssf[[i]]$y1_,condensed.dataframe.subset.remaining2$ssf[[i]]$y2_)+200)))

  points(c(condensed.dataframe.subset.remaining2$ssf[[i]]$x1_,condensed.dataframe.subset.remaining2$ssf[[i]]$x2_),c(condensed.dataframe.subset.remaining2$ssf[[i]]$y1_,condensed.dataframe.subset.remaining2$ssf[[i]]$y2_))
...
ggmap::register_google(key="AIzaSyDt6p72VHjcwRz20RXAla1DCXFwGU77w_u")

ggmap::get_map(location=c(long=-82.92,lat=8.76),maptype="satellite")


ggplot() +
  geom_raster(land_use_raster)
...
sl_shape(condensed.dataframe.subset.remaining2$fit[[1]])
```
...
land_use_raster_labeled<-Raster #Here I am replacing the original code in which I set up the patch id from Adam's file and use Felipe's shape raster map
...
for (i in 1:length(forest.raster.cropped)){
  ...
  forest.raster.cropped[[i]]$forest.raster.cropped <- crop(land_use_raster, amt::bbox(condensed.dataframe.subset$trk[[i]], spatial = TRUE, buff = 5e3))
  ...
  #JUST UNCHECKED THIS
  forest.raster.cropped[[i]]$forest.raster.cropped.labeled<-crop(land_use_raster_labeled, amt::bbox(condensed.dataframe.subset$trk[[i]], spatial = TRUE, buff = 5e3)) #Here I am using the same extent than the land_use_raster map that was cropped but with Felipe's shp raster file
  ...
  #NULL MODEL 1. OBTAIN THE AREA OF THE SAME PATCH, MATRIX AND NEW PATCH IN AN A RADIUS (WHICH CORRESPONDS TO HALF OF THE LARGEST DISTANE BETWEEN TWO PINT IN THE HOME RANGE SIZE OF ALL BIRDS)
  
  plot(land_use_raster_labeled,main=paste0("Freq ",forest.raster.cropped[[i]]$id.freq,". Map ",i))
  points(forest.raster.cropped[[i]]$focal.patch.xy)


  #I will obtain the raster map with the buffer of a/2 that I estimated from the first chapter
  #Off is the amount of meters I would like to expand from the xy coordinates of the focal patch
  off<-3500
  
  #Make an extent object to crop the map to an specific extent
  ext<-extent(forest.raster.cropped[[i]]$focal.patch.xy[,"X"]-off,forest.raster.cropped[[i]]$focal.patch.xy[,"X"]+off,forest.raster.cropped[[i]]$focal.patch.xy[,"Y"]-off,forest.raster.cropped[[i]]$focal.patch.xy[,"Y"]+off)
  
  #Crop full raster labeled map
  map.only.focal.patch<-crop(land_use_raster_labeled,ext)
  
  #If the id of the labeled raster map is different from the id of the focal patch, then set to NA
  map.only.focal.patch[map.only.focal.patch!=forest.raster.cropped[[i]]$focal.patch.label.complete.map]<-NA
  
  #Make a buffer around the focal patch equal to a/2 (value obtained from the first chapter)
  a.distance<-1695.15
  map.only.focal.patch<-buffer(map.only.focal.patch,width=a.distance/2)
  #map.only.focal.patch2<-buffer(map.only.focal.patch,width=3000)
  
  #Obtained the cropped raster map
  forest.raster.cropped[[i]]$forest.raster.cropped.labeled.null.model.1<-land_use_raster_labeled*map.only.focal.patch
  names(forest.raster.cropped[[i]]$forest.raster.cropped.labeled.null.model.1)<-"forest27"
  #forest.raster.cropped[[i]]$forest.raster.cropped<-land_use_raster_labeled*map.only.focal.patch2
  #names(forest.raster.cropped[[i]]$forest.raster.cropped)<-"forest27"
  #forest.raster.cropped[[i]]$forest.raster.cropped.labeled<-land_use_raster_labeled*map.only.focal.patch2
  #names(forest.raster.cropped[[i]]$forest.raster.cropped.labeled)<-"forest27"
  
  #Plot cropped raster map
  plot(forest.raster.cropped[[i]]$forest.raster.cropped.labeled.null.model.1,main=paste0("Freq ",forest.raster.cropped[[i]]$id.freq,". Map ",i))
  points(forest.raster.cropped[[i]]$focal.patch.xy)
  
  
  #The next step is to obtain the proportion of forest of the focal patch, the matrix and the new patch
  #Saving the cropped map in an object
  sum.total.landscape<-prop.matrix<-prop.new.patch<-prop.same.patch<-forest.raster.cropped[[i]]$forest.raster.cropped.labeled.null.model.1
  
  #All values that have a number will be set to 0 so that I can obtain the total area in the buffer
  sum.total.landscape[sum.total.landscape>=0]<-1
  sum.total.landscape<-sum(sum.total.landscape[],na.rm=T)
  
  #Now I will obtain the proportion of forest in the same patch
  #Set to 0 any value that is different from the focal patch
  prop.same.patch[prop.same.patch!=forest.raster.cropped[[i]]$focal.patch.label.complete.map]<-0
  
  #Set to 1 any value that is equal to the focal patch
  prop.same.patch[prop.same.patch==forest.raster.cropped[[i]]$focal.patch.label.complete.map]<-1
  
  #Plot map
  plot(prop.same.patch,main=paste0("Freq ",forest.raster.cropped[[i]]$id.freq," focal patch. Map ",i))
  
  #Obtain the proportion of forest of the same aptch
  prop.same.patch<-sum(prop.same.patch[],na.rm=T)/sum.total.landscape
  
  #I will obtain the proportion of area that is matrix
  #Because I need to set to 1 any value that is matrix, then first I need to change any patch that is labeled with 1 to any other value. In this case, I decided to revalue 1 to 2
  prop.matrix[prop.matrix==1]<-2
  
  #Now I will set the matrix value (0) to 1 so that I can sum it
  prop.matrix[prop.matrix==0]<-1
  
  #I will set any value greater than 1 to 0
  prop.matrix[prop.matrix>1]<-0
  
  #Plot map
  plot(prop.matrix,main=paste0("Freq ",forest.raster.cropped[[i]]$id.freq," matrix. Map ",i))
  
  #OBtain the proportion fo matrix
  prop.matrix<-sum(prop.matrix[],na.rm=T)/sum.total.landscape
  
  
  #Now I will obtain the proportion of forest that is different from focal patch
  #First I will convert the matrix value (0) to the value of the focal patch because in the next lines I will set to 0 the value of the focal patch so that it is not considered in the sum
  prop.new.patch[prop.new.patch==0]<-forest.raster.cropped[[i]]$focal.patch.label.complete.map
  
  #Any patch that is different to the focal patch will be set to 1
  prop.new.patch[prop.new.patch!=forest.raster.cropped[[i]]$focal.patch.label.complete.map]<-1
  
  #Any patch that is equal to the focal patch will be set to 0
  prop.new.patch[prop.new.patch==forest.raster.cropped[[i]]$focal.patch.label.complete.map]<-0
  
  #Plot map
  plot(prop.new.patch,main=paste0("Freq ",forest.raster.cropped[[i]]$id.freq," new patch. Map ",i))

  #Estimate the proportion of area of new patches
  prop.new.patch<-sum(prop.new.patch[],na.rm=T)/sum.total.landscape
  
  #Make a data frame with the proportion of forest of each type of vegetation
  forest.raster.cropped[[i]]$prop.landscapes.null.model.1<-data.frame(patch.id.adam=forest.raster.cropped[[i]]$landscape.variables$Patch,
                                                                      model="null.model.1",
                                                                      prop.same.patch=prop.same.patch,
                                                                      prop.matrix=prop.matrix,
                                                           prop.new.patch=prop.new.patch)
  
  print(forest.raster.cropped[[i]]$prop.landscapes.null.model.1)
  
  print(paste("Patch ",i," of ",length(forest.raster.cropped)," patches. Freq ",forest.raster.cropped[[i]]$id.freq,sep=""))
  
  #Printing information
  print(sum(forest.raster.cropped[[i]]$prop.landscapes.null.model.1[,c("prop.same.patch","prop.matrix","prop.new.patch")]))

}

  #PROBLEM WITH i 11 FREQUENCY 942: The patch is on the border of the map and it is big, so it is taking area outside the border when I make a buffer around it and considering that area as matrix


rm(structure,map.only.focal.patch,sum.total.landscape,prop.same.patch,prop.matrix,prop.new.patch,ext,off)
```
...
#GGpairs of parameters of log transformed variables
parameters.simulations %>% 
  inner_join(frequencies.patches %>% 
               mutate(Patch=PATCH,
                      id.simulation=FREQ) %>% 
  ...
  inner_join(patches.info,by="Patch") %>% 
  mutate(log.patch.size=patch_size,
         log.shape.step.length=log(shape.step.length),
         log.scale.step.length=log(scale.step.length),
         log.habitat.utilization=log(habitat.utilization)) %>% 
  select(-c(patch_size,id.simulation,Patch,shape.step.length,scale.step.length,habitat.utilization)) %>% 
  ggpairs(upper=list(continuous="cor"),lower=list(continuous="smooth"))

#Scatter plot of parameters patch size and percent of forest of sampled patches.The horizontal line delimits small and big patches
parameters.simulations %>% 
  inner_join(frequencies.patches %>% 
               mutate(Patch=PATCH,
                      id.simulation=FREQ) %>% 
  ...
  mutate(log.patch.size=patch_size,
         ...
         log.scale.step.length=log(scale.step.length),
         log.habitat.utilization=log(habitat.utilization)) %>% 
  select(c(patch_size,percent_forest)) %>% 
  ggplot(aes(x=patch_size,y=percent_forest)) +
  geom_point() +
...
#I DO NOT SEE ANY DRASTIC CHANGES, SO I COULD USE THE MEDIAN OF THE SHAPE PARAMETER

rm(scale.parameter.modification)
```

```{r}
#Boxplot parameter of habitat utilization
boxplot(parameters.simulations$habitat.utilization)

#Make data frame that will include all the combinations of the parameters I would like to simulate per patch
parameters.simulations.model <- data.frame(shape.step.length=mean(parameters.simulations$shape.step.length),
             scale.step.length=quantile(parameters.simulations$scale.step.length,probs=c(0.05,0.5,0.95)),
             habitat.utilization=quantile(parameters.simulations %>% filter(habitat.utilization>-60) %>% select(habitat.utilization) %>% .$habitat.utilization,probs=c(0.05,0.5,0.99))) %>% 
...
for (i in 1:length(forest.raster.cropped)){
  ...
  points(forest.raster.cropped[[i]]$focal.patch.xy,
         col=plasma(length(forest.raster.cropped),0.4)[i],pch=16)
 ...
 #Plot point of a specific sampled patch
  ...
  points(forest.raster.cropped[[22]]$focal.patch.xy,pch=16)
...
```{r}
#Save workspace for simulation in calculon
#save.image("workspace_calculon.RData")
...
#1,2,4,8,13,15,17,18
#for (1:length(forest.raster.cropped)){

  results.simulations.list <- foreach (i= c(1:length(forest.raster.cropped)),#1:6,8,9,11:20,22:23), #c(1:6,8,9,11:20,22:23), #11 #1:nrow(parameters.simulations),
                                .packages=c("amt","raster")) %dopar% {

simulate.movement(focal.patch=forest.raster.cropped[[i]]$focal.patch.label.cropped,
                  tot.num.minutes=tot.num.minutes,
                  fix.rate=fix.rate,
...
#Save results
#save.image("borrar.Rdata"). Saved worskpace as "simulations SSF.Rdata"
...
results.simulations.list <- c(dget("results.simulations slots 1 11.list"), dget("results.simulations slots 12 24.list"))
```

```{r}
results.simulations.df<-data.frame(matrix(0,
                                          ncol=ncol(time.location.regurgitation[[1]][[1]][[1]][[1]]),
                                          nrow=1)) 

#Name columns
  names(results.simulations.df)<-names(time.location.regurgitation[[1]][[1]][[1]][[1]])
  
  for (z in 1:length(time.location.regurgitation)){
    
    for(a in 1:length(time.location.regurgitation[[z]])){
      
      for (g in 1:length(time.location.regurgitation[[z]][[a]])){
        
        for (l in 1:length(time.location.regurgitation[[z]][[a]][[g]]))
            
      {
        
        df<-time.location.regurgitation[[z]][[a]][[g]][[l]]
...
#***ADDED THIS CHUNK OF CODE. IT IS THE CODE I HAVE FOR THE FUNCTION raw.dataframe BUT OUTSIDE IS WORKING
  ...
  results.simulations.df<-data.frame(matrix(0,
                                          ncol=ncol(results.simulations.list[[1]][[1]][[1]][[1]][[1]][[1]]),
                                          nrow=1)) 
  
  #Name columns
  names(results.simulations.df)<-names(results.simulations.list[[1]][[1]][[1]][[1]][[1]][[1]])


  #Paste all data bases
  for (z in 1:length(results.simulations.list)){
    
    for(a in 1:length(results.simulations.list[[z]][[1]])){
      
      for (g in 1:length(results.simulations.list[[z]][[1]][[a]])){
        
        for (l in 1:length(results.simulations.list[[z]][[1]][[a]][[g]]))
          
          for (q in 1:length(results.simulations.list[[z]][[1]][[a]][[g]][[l]]))
            
      {
        
        df<-results.simulations.list[[z]][[1]][[a]][[g]][[l]][[q]]
...
```{r}
#Join the data frame of the lists of points that landed in a place in a mpa where there is no information
new.coords.test<-data.frame(matrix(0,
                                          ncol=ncol(results.simulations.list[[z]][[2]]),
                                          nrow=1)) 
  
  #Name columns
  names(new.coords.test)<-names(results.simulations.list[[z]][[2]])

#Paste all data bases
  for (z in 1:length(results.simulations.list)){
    
    df<-results.simulations.list[[z]][[2]]
        
        new.coords.test<-rbind(df,new.coords.test)
        
      }

  #Count the proportion ofpoints that landed in a place in a mpa where there is no information
  new.coords.test %>% 
    filter(starting.coord.x!=0) %>% 
    mutate(test=is.na(test.in.new.coord)) %>% 
    group_by(test) %>% 
    summarise(total=n()) %>% 
    ungroup() %>% 
    mutate(proportion=total/nrow(new.coords.test %>% 
    filter(starting.coord.x!=0)))
...
```{r}
plot(land_use_raster)
points(results.simulations.df$x,results.simulations.df$y,col=results.simulations.df$patch.id.adam)
```

```{r}
#I will plot the points of the simulation in the landscape per patch
for (i in 1:length(forest.raster.cropped)){
  
  #Plot forest map
  plot(forest.raster.cropped[[i]]$forest.raster.cropped,main=paste("Patch ",i))
  
  #Subset the points from that patch
  p<-results.simulations.df %>% 
    filter(patch.id.adam==forest.raster.cropped[[i]]$landscape.variables$Patch)
...
```{r}
forest.raster.cropped[[21]]$landscape.variables$Patch

results.emigration<-results.emigration %>% 
  filter(patch.id.adam!=53) %>% 
  filter(patch.id.adam!=32) #%>% 
  #filter(patch.id.adam!=40)
```


```{r}
#I will do a chi test to each of the simulation I made
#I need a data frame with the proportion of vegetation forest for each sampled patch
prop.null.model.1<-forest.raster.cropped[[1]]$prop.landscapes.null.model.1

for (i in 2:length(forest.raster.cropped)){

prop.null.model.1<-rbind(prop.null.model.1,forest.raster.cropped[[i]]$prop.landscapes.null.model.1)
  
}


#Data frame with information for chi test
chi.test.prop<-results.emigration %>% 
  select(patch.id.adam,time,prop.same.patch,prop.matrix,prop.new.patch) %>% 
  add_column(model = "simulation", .after = "patch.id.adam") %>% 
  mutate(p.value=0)
  #bind_rows(prop.null.model.1) 

#chi.test.prop[is.na(chi.test.prop)=="TRUE"]<-0


#Do the chi test for each of the rows (which reflects each of the simulations)
for (i in 1:nrow(chi.test.prop)){
  
  #Obtain the id of the patch that Adam gave of the line to analyze
  patch<-chi.test.prop[i,"patch.id.adam"]
  
  #Select the line
  d<- chi.test.prop[i,] %>% 
    select("prop.same.patch","prop.matrix","prop.new.patch") #%>% 
    #bind_rows(prop.null.model.1 %>% 
     #           filter(patch.id.adam==patch) %>%
      #          select("prop.same.patch","prop.matrix","prop.new.patch") %>% 
       #         distinct())
 
  #Do the chi test and store the value
  chi.test.prop[i,"p.value"]<-chisq.test(as.vector(t(d)),p=t(prop.null.model.1 %>% 
                filter(patch.id.adam==patch) %>%
                select("prop.same.patch","prop.matrix","prop.new.patch") %>% 
                distinct()))$p.value #The reference distribution is the proportion of each vegetal cover in each landscape, so I am obtaining that information for each patch
  
}

chi.test.prop %>% filter(p.value>0.05)
...
summary(test.proportions<-results.emigration %>% 
  ...
  gather(fate,proportion,prop.same.patch:prop.new.patch) %>% 
    mutate(fate=relevel(factor(fate), ref = 3)) %>% #Re leveling factors so that I can compare proportion of seeds that landed in the same patch with the other ones
...
results.emigration %>% 
  mutate(density.same.patch=same.patch/patch.size) %>% 
  select(-c(focal.patch,patch.id.adam,param.shape.step.length,matrix,new.patch,same.patch)) %>% 
  mutate(patch.size=log(patch.size)) %>% 
  rename(log.patch.size=patch.size) %>% 
  ggpairs(upper=list(continuous="cor"),lower=list(continuous="smooth"))

results.emigration<-results.emigration %>% 
  mutate(log.patch.size=log(patch.size),
         density.same.patch=same.patch/patch.size)

#Proportion of fruits in the same patch
#Model without random structure
library("nlme")
summary(mod.1<-gls(asin(sqrt(prop.same.patch)) ~scale(param.scale.step.length) + scale(param.habitat.utilization) + scale(time) + scale(log.patch.size) + scale(percent.forest) + scale(elevation),method = "REML", data = results.emigration %>% mutate(id=paste(patch.id.adam,focal.patch)),na.action=na.omit))


#GLMM nesting frequency
summary(mod.2<-lme(asin(sqrt(prop.same.patch)) ~scale(param.scale.step.length) + scale(param.habitat.utilization) + scale(time) + scale(log.patch.size) + scale(percent.forest) + scale(elevation),random = ~1 | id,data=results.emigration %>% mutate(id=paste(patch.id.adam,focal.patch),method="REML"),na.action=na.omit))

#Continue with LMM
#Without step length
summary(mod.3 <- update(mod.2,.~. -scale(percent.forest)))
...
#Model without random structure
library("nlme")
summary(mod.1<-gls(asin(sqrt(prop.matrix)) ~scale(param.scale.step.length) + scale(param.habitat.utilization) + scale(time) + scale(log.patch.size) + scale(percent.forest) + scale(elevation),method = "REML", data = results.emigration %>% mutate(id=paste(patch.id.adam,focal.patch))))


#GLMM nesting frequency
summary(mod.2<-lme(asin(sqrt(prop.matrix)) ~scale(param.scale.step.length) + scale(param.habitat.utilization) + scale(time) + scale(log.patch.size) + scale(percent.forest) + scale(elevation),random = ~1 | id,data=results.emigration %>% mutate(id=paste(patch.id.adam,focal.patch),method="REML")))
...
#Without patch size
summary(mod.4 <- update(mod.3,.~. -scale(elevation)))
...
#Model without random structure
library("nlme")
summary(mod.1<-gls(asin(sqrt(prop.new.patch)) ~scale(param.scale.step.length) + scale(param.habitat.utilization) + scale(time) + scale(log.patch.size) + scale(percent.forest) + scale(elevation),method = "REML", data = results.emigration %>% mutate(id=paste(patch.id.adam,focal.patch))))


#GLMM nesting frequency
summary(mod.2<-lme(asin(sqrt(prop.new.patch)) ~scale(param.scale.step.length) + scale(param.habitat.utilization) + scale(time) + scale(log.patch.size) + scale(percent.forest) + scale(elevation),random = ~1 | id,data=results.emigration %>% mutate(id=paste(patch.id.adam,focal.patch),method="REML")))
...
#AICc
AICc(mod.1,mod.2,mod.3,mod.4,mod.5,mod.6)
...
results.emigration %>% 
  ggplot(aes(x=param.scale.step.length,y=prop.new.patch)) +
  geom_point() +
  geom_smooth(method="lm")

results.emigration %>% 
  ggplot(aes(x=param.habitat.utilization,y=prop.new.patch)) +
  geom_point() +
  geom_smooth(method="lm")
...
#Create a data frame with 1 row and the number of columns so that it matches the data frame 
  ...
  #Name columns
  names(results.simulations.dispersal.distance.df)<-names(results.simulations.list[[1]][[1]][[1]][[1]][[1]][[1]])
...
results.simulations.dispersal.distance.df%>% 
  ...
  select(log.dispersal.distance,param.scale.step.length,param.habitat.utilization,time,log.patch.size,percent.forest,elevation) %>%
  ggpairs(upper=list(continuous="smooth"),lower=list(continuous="cor"))
...
#Model without random structure
#library("nlme")
summary(mod.1<-gls(log.dispersal.distance ~scale(param.scale.step.length) + scale(param.habitat.utilization) + scale(time) + scale(log.patch.size) + scale(percent.forest) + scale(elevation),method = "REML", data = results.simulations.dispersal.distance.df %>% 
  filter(same.patch==1 | new.patch==1) %>% mutate(id=paste(patch.id.adam,focal.patch),
                                                   log.dispersal.distance=log(dispersal.distance+1)),na.action=na.omit))


#GLMM nesting frequency
summary(mod.2<-lme(log.dispersal.distance ~scale(param.scale.step.length) + scale(param.habitat.utilization) + scale(time) + scale(log.patch.size) + scale(percent.forest) + scale(elevation),random = ~1 | id,data=results.simulations.dispersal.distance.df %>% 
  filter(same.patch==1 | new.patch==1) %>% mutate(id=paste(patch.id.adam,focal.patch),
...
summary(mod.1<-gls(log.dispersal.distance ~scale(param.scale.step.length) + scale(param.habitat.utilization) + scale(time) + scale(log.patch.size) + scale(percent.forest) + scale(elevation),method = "REML", data = results.simulations.dispersal.distance.df %>% 
  filter(new.patch==1)) %>% mutate(id=paste(patch.id.adam,focal.patch),
                                                            log.dispersal.distance=log(dispersal.distance+1)),na.action=na.pass)
...
summary(rda(results.simulations.dispersal.distance.df[,c("same.patch","matrix","new.patch")],results.simulations.dispersal.distance.df[,c("param.scale.step.length","param.habitat.utilization","time","log.patch.size","percent.forest","elevation")]))
...
#load("borrar.RData")
for (i in 1:length(forest.raster.cropped)){
#for (i in c(1,2,4,8,9)){ #not included: 5
#for (i in c(1:6,12,14,15,18,20,24)){ #included: 1:6,12,14,15,18,20,24
start <- Sys.time()
  print(paste("Starting slot ",i,". Time ",start))
...
forest.raster.cropped[[i]]$forest.raster_labeled_immigration[forest.raster.cropped[[i]]$forest.raster_labeled_immigration !=forest.raster.cropped[[i]]$focal.patch.label.complete.map]<-NA
...
Buffer <- buffer(forest.raster.cropped[[i]]$forest.raster_labeled_immigration, width=a.distance)
...
forest.raster.cropped[[i]]$focal.patch.immigration<-forest.raster.cropped[[i]]$focal.patch.label.complete.map
...
forest.raster.cropped[[i]]$patches.immigration<-unique(forest.raster.cropped[[i]]$forest.raster.cropped.labeled.null.model.1[])
...
Run replicate simulations:
##***I THINK IS SHOULD SELECT THE PATCHES FROM THE RASTER.CROPPED.MAP.BUFFER AND THEN DO A BUFFER FROM THE LIMITS OF THOSE PATCHES SO THAT, WHEN I SIMULATE THE MOVEMENT BEHAVIOR, THEY CAN MOVE FREELY IN THE LANDSCAPE AND THEY DO NOT BOUNCE
```{r}
#Save workspace for simulation in calculon
#save.image("workspace_calculon_immigration.RData")

source("SSF simulation functions all birds.R")
#load("immigration.database.Rdata")
...
#1,2,4,8,13,15,17,18
#for (i in 1:length(forest.raster.cropped)){
#for (i in c(1:10,12:length(forest.raster.cropped))){
...
simulate.movement(focal.patch=forest.raster.cropped[[i]]$patches.immigration,
                  focal.patch.immigration=forest.raster.cropped[[i]]$focal.patch.label.cropped,
                  tot.num.minutes=60,
                  fix.rate=5,
                  parameters.simulations=parameters.simulations.model,
                  ...
                  #I tried to only put a number and then save the number I put with the name of the variable but I could not do it, so that is why I specifya outside the function the value and store in in the variable
...
```{r}
results.simulations.list.immigration <- dget("results.simulations.immigration.patches.1.3.list")
...
#***ADDED THIS CHUNK OF CODE. IT IS THE CODE I HAVE FOR THE FUNCTION raw.dataframe BUT OUTSIDE IS WORKING
  ...
  #I generated for each simulation (in this case is called results.simulations2). I am using the
  #first element of the list as a molde
  results.simulations.df.immigration<-data.frame(matrix(0,
                                          ncol=ncol(results.simulations.list.immigration[[1]][[1]][[1]][[1]][[1]][[1]]),
                                          nrow=1)) 
  
  #Name columns
  names(results.simulations.df.immigration)<-names(results.simulations.list.immigration[[1]][[1]][[1]][[1]][[1]][[1]])


  #Paste all data bases
  for (z in 1:length(results.simulations.list.immigration)){
    
    for(a in 1:length(results.simulations.list.immigration[[z]][[1]])){
      
      for (g in 1:length(results.simulations.list.immigration[[z]][[1]][[a]])){
        
        for (l in 1:length(results.simulations.list.immigration[[z]][[1]][[a]][[g]]))
          
          for (q in 1:length(results.simulations.list.immigration[[z]][[1]][[a]][[g]][[l]]))
            
      {
        
        df<-results.simulations.list.immigration[[z]][[1]][[a]][[g]][[l]][[q]]
        
        results.simulations.df.immigration<-rbind(df,results.simulations.df.immigration)
  ...
  #Delete last observation which has nothing
  results.simulations.df.immigration<-results.simulations.df.immigration[-nrow(results.simulations.df.immigration),]
...
#Join the data frame of the lists of points that landed in a place in a mpa where there is no information
new.coords.test<-data.frame(matrix(0,
                                          ncol=ncol(results.simulations.list.immigration[[z]][[2]]),
                                          nrow=1)) 
  
  #Name columns
  names(new.coords.test)<-names(results.simulations.list.immigration[[z]][[2]])

#Paste all data bases
  for (z in 1:length(results.simulations.list.immigration)){
    
    df<-results.simulations.list.immigration[[z]][[2]]
...
#results.simulations.df.immigration<-results.simulations.df.immigration %>% 
 # mutate(time=rep(c(20,40),times=nrow(results.simulations.df.immigration)/2))

results.immigration <- final.database(name.input.database=results.simulations.df.immigration,
                                      for.immigration = "Y",
...
```{r}
#Plot all the simulated points
plot(land_use_raster)
points(results.simulations.df.immigration$x,results.simulations.df.immigration$y,col=results.simulations.df.immigration$patch.id.adam)

#Plot only points that landed in the focal patch
plot(land_use_raster)
points((results.simulations.df.immigration %>% filter(patch.emigrated==focal.patch.immigration))$x,(results.simulations.df.immigration%>% filter(patch.emigrated==focal.patch.immigration))$y,col=results.simulations.df.immigration$patch.id.adam)
```
...
for (i in 1:length(forest.raster.cropped)){
  ...
  #Subset the points from that patch
  p<-results.simulations.df.immigration %>% 
    filter(patch.id.adam==forest.raster.cropped[[i]]$landscape.variables$Patch)
...
##Obtain the dispersal distance for each fix rate
#dispersal.distance(#focal.patch=focal.patch,
 #                  name.input.database=results.simulations.df.immigration)
```

```{r}
results.immigration %>% 
  select("immigrated","param.scale.step.length","param.habitat.utilization","time","patch.size","percent.forest","elevation") %>% 
  ggpairs(upper=list(continuous="smooth"),lower=list(continuous="cor"))
```

#Joining emigration and immigration data bases for estimating emigration/immigration rates
```{r}
emigration.immigration.df<-left_join(results.emigration %>% select(-c("prop.matrix","prop.new.patch","prop.same.patch")) %>% 
                                                                      filter(time==20 | time==40 | time==60),
                                      results.immigration %>% 
                                        select(c("focal.patch","patch.id.adam","param.shape.step.length","param.scale.step.length","param.habitat.utilization","time","immigrated")),
                                      by=c("focal.patch","patch.id.adam","param.shape.step.length","param.scale.step.length","param.habitat.utilization","time")) %>% 
  mutate(immigrated=if_else(is.na(immigrated)=="TRUE",0,immigrated)) %>% 
  mutate(emigration.immigration.rate=((same.patch + immigrated)/same.patch)) %>% 
  mutate(emigration.immigration.rate=if_else(is.na(emigration.immigration.rate)=="TRUE",1,emigration.immigration.rate))


#number of seeds that stay + number of seeds that enter/number of seeds that stay

emigration.immigration.df[,c(ncol(emigration.immigration.df),1:ncol(emigration.immigration.df)-1)] %>% 
  select(-c(patch.id.adam,matrix,new.patch,same.patch,immigrated)) %>% 
  ggpairs(upper=list(continuous="smooth"),lower=list(continuous="cor"))

#Scale step lenght the only one that was significant
summary(lm(emigration.immigration.rate~param.scale.step.length +param.habitat.utilization + time,data=emigration.immigration.df))

emigration.immigration.df %>% 
  ggplot(aes(x=param.scale.step.length,y=emigration.immigration.rate,group=param.scale.step.length)) +
  geom_boxplot()
```
...
####THIS CODE I DO NOT THINK I NEED TO RUN IT FOR NOW
