...
for (i in 1:length(forest.raster.cropped)){
  ...
  #JUST UNCHECKED THIS
  #forest.raster.cropped[[i]]$forest.raster.cropped.labeled<-crop(land_use_raster_labeled, amt::bbox(condensed.dataframe.subset$trk[[i]], spatial = TRUE, buff = 1e3)) #Here I am using the same extent than the land_use_raster map that was cropped but with Felipe's shp raster file
...
summary(test.proportions<-results.emigration %>% 
  ...
  select(id,prop.same.patch,prop.matrix,prop.new.patch)  %>% 
  gather(fate,proportion,prop.same.patch:prop.new.patch) %>% 
    mutate(fate=relevel(factor(fate), ref = 3)) %>% 
  lme(asin(sqrt(proportion))~fate,random = ~1 | id,data=.))
