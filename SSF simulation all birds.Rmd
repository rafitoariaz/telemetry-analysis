...
#Model without random structure
library("nlme")
summary(mod.1<-gls(asin(sqrt(prop.same.patch)) ~scale(param.shape.step.length) + scale(param.habitat.utilization) + scale(time) + scale(log.patch.size) + scale(percent.forest) + scale(elevation),method = "REML", data = results.emigration %>% mutate(id=paste(patch.id.adam,focal.patch)),na.action=na.omit))


#GLMM nesting frequency
summary(mod.2<-lme(asin(sqrt(prop.same.patch)) ~scale(param.shape.step.length) + scale(param.habitat.utilization) + scale(time) + scale(log.patch.size) + scale(percent.forest) + scale(elevation),random = ~1 | id,data=results.emigration %>% mutate(id=paste(patch.id.adam,focal.patch),method="REML"),na.action=na.omit))
...
```{r}
emigration.immigration.df<-inner_join(results.emigration %>% select(-c("focal.patch","matrix","new.patch","same.patch")),
                                      results.immigration %>% 
                                        select(c("patch.id.adam","param.shape.step.length","param.scale.step.length","param.habitat.utilization","time","prop.immigrated")),
                                      by=c("patch.id.adam","param.shape.step.length","param.scale.step.length","param.habitat.utilization","time")) %>% 
  mutate(emigration.immigration.rate=if_else(prop.same.patch/prop.immigrated==Inf,prop.same.patch,prop.same.patch/prop.immigrated))
```
