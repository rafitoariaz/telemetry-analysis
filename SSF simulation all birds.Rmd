...
condensed.dataframe.subset <- rbind(condensed.dataframe.subset,condensed.dataframe.subset.remaining)
```
...
summary(lm(shape.step.length~scale.step.length,data=parameters.simulations))

scale.parameter.modification <- parameters.simulations %>% 
  mutate(original=scale.step.length,
    lower=quantile(scale.step.length,probs=0.05),
         median=median(scale.step.length),
         upper=quantile(scale.step.length,probs=0.95))
...
#TASKS TO DO.
#1.-SEE HOW THE SCALE PARAMETER MODIFIES THE GAMMA DISTRIBUTION
#2. CHECK WHY SOME INDIVIDUALS I CANNOT FIT A SSF
#3. MODIFY SSF FUNCTTIONS OF MOVEMENT SIMULATION SO THAT I CAN SIMULATION EVERY COMBINATION OF THE PARAMETERS
