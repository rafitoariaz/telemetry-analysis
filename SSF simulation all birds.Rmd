...
#Make a splitted list with the id. freq. ***HERE I COULD USE THE LAND_USE_RASTER (THIS IS, THE COMPLETE MAP OF THE LANDSCAPE BUT LABALED) TO DO THE LIST SO THAT I HAVE REPRESENTED EACH PATCH OF THE LANDSCAPE. aLTERNATIVELY, FOR MY THESIS I CAN LOCATE THE PATCH IN THE LAND_USE_RASTER BASED ON THE PATCHES.LOCATION OBJECT (WHICH HAS THE XY COORDINATES OF THE PATCH WHERE THE BIRD WAS CAPTURED, SEE BELOW) TO SELECT ONLY THOSE PATCHES AND DO THE SIMULATIONS ONLY IN THOSE PATCHES
...
for (i in 1:length(forest.raster.cropped)){
  ...
  #Put the id. freq. ***HERE I WOULD PUT THE PATCH ID. I THINK I AM PUTTING THE ID.FREQ BECAUSE IT IS BASED ON WHERE THE BIRD MOVED, 
  forest.raster.cropped[[i]]$id.freq <- condensed.dataframe.subset$id[[i]] 
  
  #Crop the map to the area in which the animal moved. #****HERE I COULD CROP THE MAP BASED ON THE LOCATION OF THE PATCH WHERE THE BIRD MOVED
  ...
  #Sotre forest raster labaled. #****HERE I WOULD PUT ANY CROPPED MAP BUT LABELED
...
focal.patch.database<-data.frame(id=condensed.dataframe.subset$id)

focal.patch.database$id <- factor(focal.patch.database$id,levels=unique(focal.patch.database$id))

focal.patch.database <- split(focal.patch.database$id,focal.patch.database$id)
...
#JUST ADDED THIS LINE. I THINK IT WOULD BE EASIER IF I PUT ON MY DATA BASE OF THE RASTER THE INFORMATION OF THE FOCAL PATCH LABEL ID
...
for (i in 1:length(forest.raster.cropped)){
  ...
  forest.raster.cropped[[i]]$focal.patch.label.complete.map <- land_use_raster_labeled[cellFromXY (land_use_raster_labeled, focal.patch.database[[i]]$focal.patch.xy)]
  ...
  forest.raster.cropped[[i]]$focal.patch.label.cropped <- ConnCompLabel(forest.raster.cropped[[i]]$forest.raster.cropped)[cellFromXY (ConnCompLabel(forest.raster.cropped[[i]]$forest.raster.cropped), focal.patch.database[[i]]$focal.patch.xy)]
...
#Make a list of the
results.simulations <- split(parameters.simulations$num.simulation,parameters.simulations$num.simulation)
...
system.time(
  results.simulations2 <- foreach (i=c(1,2,4,9,11,13,14),#c(1,4),
