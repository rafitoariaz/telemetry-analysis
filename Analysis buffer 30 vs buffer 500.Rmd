...
#Make variable of transfromed data
points.info$SQRT.PROP.FOR.500<-sqrt(asin(points.info$PROP.FOR.500))
points.info$LOG.diff<-log(points.info$diff+1)

pairs(points.info[c("SQRT.PROP.FOR.500","LOG.diff")],upper.panel=panel.cor,diag.panel=panel.hist,panel=panel.smooth,main="Raw data")


model3<-lme(LOG.diff~SQRT.PROP.FOR.500,random= ~1|PATCH_ID/FREQ, data=points.info)
model4<-lme(LOG.diff~SQRT.PROP.FOR.500,random= ~PROP.FOR.500|PATCH_ID/FREQ, data=points.info)
...
#MODEL FOR PROP FORES IN 30 METERS BUFFER AS A RESPONSE VARIABLES
summary(model5<-lme(PROP.FOR.30~PROP.FOR.500,random= ~1|PATCH_ID/FREQ, data=points.info)) #Random slope
summary(model6<-lme(PROP.FOR.30~PROP.FOR.500,random= ~PROP.FOR.500|PATCH_ID/FREQ, data=points.info)) #Random slple and intercept

points.info %>% 
  ggplot(aes(x=PROP.FOR.500,y=PROP.FOR.30,color=FREQ)) +
  geom_point() + 
  #geom_jitter(position = position_jitter(width = 0.1, height = 0.1)) +
  geom_smooth(method="lm",se=F) +
    geom_abline(intercept=model6$coefficients$fixed[1], slope=model6$coefficients$fixed[2],size=1)

plot(model6)

summary(model7<-lme(sqrt(asin(PROP.FOR.30))~sqrt(asin(PROP.FOR.500)),random= ~1|PATCH_ID/FREQ, data=points.info)) #Random slope
summary(model8<-lme(sqrt(asin(PROP.FOR.30))~sqrt(asin(PROP.FOR.500)),random= ~PROP.FOR.500|PATCH_ID/FREQ, data=points.info)) #Random slple and intercept

points.info %>% 
  ggplot(aes(x=sqrt(asin(PROP.FOR.500)),y=sqrt(asin(PROP.FOR.30)),color=FREQ)) +
  geom_point() +
  geom_smooth(method="lm",se=F)
...
#Do plot with points for mean proportion of forest in used vs available
...
################################################
#Merge databases of mean proportion of forest in used areas (30m buffer) with proportion of forest in 
#buffer equal to the largest home range
##########################################
...
intercept<-data.frame(FREQ=as.numeric(),intercept=as.numeric(),MEAN.PROP.FOR.500=as.numeric(),p.value=as.numeric(),significant=factor(levels=c("NO","YES")),upper=as.numeric(),lower=as.numeric())#,adj.r.squared=as.numeric())

for(i in unique(points.info$FREQ)){
  
  intercept[i,"FREQ"]<-i
  intercept[i,"intercept"]<-models.each.bird[[i]]$coefficients[1]
  intercept[i,"MEAN.PROP.FOR.500"]<-points.info %>% filter(FREQ==i) %>% summarise(mean(PROP.FOR.500))
  intercept[i,"p.value"]<-summary(models.each.bird[[i]])$coefficients[1, 4]
  intercept[i,"significant"]<-ifelse(intercept[i,"p.value"]>0.05,"NO","YES")
  intercept[i,"upper"]<-intercept[i,"intercept"]+summary(models.each.bird[[i]])$coefficients[1, 2]
  intercept[i,"lower"]<-intercept[i,"intercept"]-summary(models.each.bird[[i]])$coefficients[1, 2]
...
intercept %>% 
  filter(p.value!=1) %>% 
  ggplot(aes(x=MEAN.PROP.FOR.500,y=intercept),group=significant,color=significant) + 
...
hist(intercept$intercept)

t.test(x=intercept,mu=0,alternative="two.sided")

qqPlot(intercept$intercept)
shapiro.test(intercept$intercept)



slope<-data.frame(slope=as.numeric())

for(i in unique(points.info$FREQ)){
  
  slope[i,1]<-models.each.bird[[i]]$coefficients[2]
