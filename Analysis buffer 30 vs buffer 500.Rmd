...
#setwd("Z:/Data bases")
#setwd("/media/ResNAS/Data bases")

#Source functions
#source("Z:/R code/Telemetry_analysis_clay_colored_thrush_functions.R")
#source("Telemetry_analysis_clay_colored_thrush_functions_a_method.R")
...
#points.info <- data.frame(matrix(NA, 
#                nrow=nrow(prueba), ncol=4, 
#                dimnames=list(1:nrow(prueba), c("PATCH_ID", "FREQ",
#                         "PROP.FOR.30", "PROP.FOR.500"))))
...
#pairs(points.info[c("PROP.FOR.500","PROP.FOR.30")],upper.panel=panel.cor,diag.panel=panel.hist,panel=panel.smooth,main="Raw data")

#The data seems not normally distributed. See if log transformation helps to normalize data. I DO NOT KNOW HOW TO TRANSFORM THESE DATA, FOR NOW I WILL CONTINUE
```
...
library("nlme")
model1<-lme(diff~PROP.FOR.500,random= ~1|PATCH_ID/FREQ, data=points.info)
#model2<-lme(PROP.FOR.30~PROP.FOR.500,random=~1|FREQ,data=points.info)
...
points.info$SQRT.PROP.FOR.500<-sqrt(points.info$PROP.FOR.500)
...
model2<-lme(log(diff+1)~sqrt(PROP.FOR.500),random= ~1|PATCH_ID/FREQ, data=points.info)
#model2<-lme(PROP.FOR.30~PROP.FOR.500,random=~1|FREQ,data=points.info)
...
```{r}
hist(mean_prop_for_ind$mean_diff_prop_for_ind)
```