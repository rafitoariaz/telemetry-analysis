...
#Source functions
source("Clean SSF data base.R") #I just dpulicated the code I used for cleaning the telemetry data just in case I modified the latter, so to avoid that the changes affect the present code 
...
Plot the map and add individual simulated paths:
```{r}

source("SSF simulation functions.R")
individual.map.simulations(focal.patch=focal.patch,
                num.sim.points.per.patch=num.sim.points.per.patch,
                num.sim.per.point=num.sim.per.point,
                num.map.start=20,
                num.map.end=40,
...
#Plot stacked bar graph of individuals that were changed in their patch
```{r}
correct_order <- c('Small isolated', 'Small connected', 'Large connected')
correct_order_original_patch <- c('SNF', 'N', 'SF')


#Read file with seed faith according to bird id
read.xlsx("Results emigration.xlsx",sheet="Res emigration moved animals") %>% 
  select(focal.patch.original,focal.patch.new,time,prop.same.patch,prop.matrix,prop.new.patch) %>% 
  filter(time==20 | time==40 | time==60) %>% 
  #mutate(time=as.factor(time),
  mutate(time=if_else(time==20,"20 minutes",as.character(time)),
         time=if_else(time=="40","40 minutes",time),
         time=if_else(time=="60","60 minutes",time),
         focal.patch.new=if_else(focal.patch.new==39,"Small isolated",as.character(focal.patch.new)),
         focal.patch.new=if_else(focal.patch.new=="13","Small connected",focal.patch.new),
         focal.patch.new=if_else(focal.patch.new=="1","Large connected",focal.patch.new),
         focal.patch.original=if_else(focal.patch.original==39,"SNF",as.character(focal.patch.original)),
         focal.patch.original=if_else(focal.patch.original=="13","N",focal.patch.original),
         focal.patch.original=if_else(focal.patch.original=="1","SF",focal.patch.original)) %>%
  #focal.patch.new=if_else(focal.patch.new==39,"Small isolated",as.character(focal.patch.new)),
  #focal.patch.new=if_else(focal.patch.new=="13","Small connected",focal.patch.new),
  #focal.patch.new=if_else(focal.patch.new=="1","Large connected",focal.patch.new)) %>% 
  gather(Fate,proportion,prop.same.patch,prop.matrix,prop.new.patch) %>% #Needed to make data base in a long format
  mutate(Fate=if_else(Fate=="prop.matrix","Matrix",Fate),
         Fate=if_else(Fate=="prop.new.patch","New patch",Fate),
         Fate=if_else(Fate=="prop.same.patch","Same patch",Fate),
         focal.patch.original = factor(focal.patch.original, levels = correct_order_original_patch)) %>% #Putting levels on factors so that they are in the order I want in the graph
  mutate(focal.patch.new = factor(focal.patch.new, levels = correct_order)) %>%
  ggplot(aes(x=focal.patch.original,y=proportion)) +
  geom_col(aes(fill=Fate)) +
  facet_wrap(~focal.patch.new+time) +
  theme(axis.text.x=element_text(angle = 45, hjust = 1)) +
  xlab("Habitat selection") +
  ylab("Proportion of seeds") +
  scale_fill_manual(values = c("#336600","#4C9900","#B2FF66")) +
  #scale_fill_brewer(palette = "RdYlGn") +
  theme_bw(base_size=150) + #Size of all letters in the plot big
  theme(legend.position="top",
        legend.title=element_blank(), #Take out legend title
        legend.justification = "center",
        legend.spacing.x=unit(5, 'cm'), #Space between levels in legend 
        legend.key.size = unit(3, "cm")) #Width of the key colors in the legend

ggsave("Graphs/Results birds changed.jpg",width=60,height=60,units="in",limitsize = F)


```


#Plot stacked bar graph of individuals that were changed in their patch for Switzerland presentation
```{r}
correct_order <- c('Small isolated', 'Small connected', 'Large connected')
correct_order_original_patch <- c('SNF', 'N', 'SF')
...
read.xlsx("Results emigration.xlsx",sheet="Res emigration moved animals") %>% 
  select(focal.patch.original,focal.patch.new,time,prop.same.patch,prop.matrix,prop.new.patch) %>% 
  filter(time==40) %>% 
  mutate(time=if_else(time==40,"40 minutes",as.character(time)),
         focal.patch.new=if_else(focal.patch.new==39,"Small isolated",as.character(focal.patch.new)),
         focal.patch.new=if_else(focal.patch.new=="13","Small connected",focal.patch.new),
         focal.patch.new=if_else(focal.patch.new=="1","Large connected",focal.patch.new),
         focal.patch.original=if_else(focal.patch.original==39,"SNF",as.character(focal.patch.original)),
  ...
  theme(#legend.position="top",
        legend.title=element_blank(), #Take out legend title
        legend.justification = "center",
        legend.key.size = unit(6, "cm")) #Width of the key colors in the legend

ggsave("Graphs/Results birds changed Switzerland.jpg",width=70,height=25,units="in",limitsize = F)
